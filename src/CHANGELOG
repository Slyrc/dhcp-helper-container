Version 1.3   Add -w flag to wait before starting actual execution
              this is helpful for deployments inside Kubernetes/Docker
              as the pod and sometimes the DHCP server itself needs some
              more time to be ready.

version 1.2   Add -n flag to inhibit daemonisation. Thanks to 
              Andrey Alexandrenko for the patch.
              Cope with the interface to the server not having an IP
              address. Thanks to Dorian for the patch.
